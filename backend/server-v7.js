...
const APP_VERSION = "7.1.1-environment-learning";
const PORT = 8094;
...
function evaluateScalp(symbol) {
  ...
  const bullishCandle = mom3 > 0;
  const bearishCandle = mom3 < 0;
  const volumeOk = true;
...
function evaluateStrategy(symbol) {
  ...
  const settings = { ...state.settings, ...state.settings.strategy, allowLong: state.settings.allowLong, allowShort: state.settings.allowShort };
  const price = priceObj.price;
  const sr = supportResistance(symbol);
  const btc = btcRegime();
  const mom5 = momentumPercent(symbol, 5);
...
const currentVol = Number(state.prices[symbol]?.volume || 0);
const volumeOk = avgVol > 0 ? currentVol >= avgVol * 1.1 : true;

const lastPrices = recent.map(x => Number(x.price || 0)).filter(Boolean);
const last = lastPrices[lastPrices.length - 1] || price;
const prev = lastPrices[lastPrices.length - 2] || price;
const prev3 = lastPrices[lastPrices.length - 4] || price;

const bullishCandle = last > prev && last > prev3;
const bearishCandle = last < prev && last < prev3;

const btcOkLong = btc !== "BTC_RISK_OFF";
const btcOkShort = true;
...
if (longScore >= 70) {
  ...
  reason = `High quality LONG: near support, momentum rising, candle ${bullishCandle ? "bullish" : "weak"}, volume ${volumeOk ? "confirmed" : "weak"}, BTC ${btc}`;
}

if (shortScore >= 70) {
  signal = "SHORT";
  confidence = shortScore;
  reason = `High quality SHORT: near resistance, momentum falling, candle ${bearishCandle ? "bearish" : "weak"}, volume ${volumeOk ? "confirmed" : "weak"}, BTC ${btc}`;
}
  ...
  // Final strategy signal enforcement
  if (signal === "HOLD" && shortScore >= 70) {
    signal = "SHORT";
    confidence = shortScore;
    reason = `Forced SHORT from strategy score ${shortScore}`;
  }

  if (signal === "HOLD" && longScore >= 70) {
    signal = "LONG";
    confidence = longScore;
    reason = `Forced LONG from strategy score ${longScore}`;
  }
