...
function evaluateStrategy(symbol) {
  ...
  const sr = candleSR(symbol, "15m", 20);
  const m5 = candleMomentum(symbol, "5m", 3);
  ...
  let signal = "HOLD";
  let confidence = 50;
  let reason = "Watching 5m/15m structure";

  if (m5 > 0 && m15 > 0 && state.settings.strategy.allowLong) {
    signal = "LONG";
    confidence = 72;
    reason = "5m and 15m trend up";
  }

  if (m5 < 0 && m15 < 0 && state.settings.strategy.allowShort) {
    signal = "SHORT";
    confidence = 72;
    reason = "5m and 15m trend down";
  }

  if (signal === "LONG" && btc === "BTC_RISK_OFF") {
    signal = "REJECT";
    confidence = 35;
    reason = "BTC risk-off blocks long";
  }
  ...
  const data = review(symbol, "strategy", signal, reason, confidence, {
    price: p.price,
    strategy: "V8 Candle Strategy",
    expectedNetGbp: ex,
    support: sr.support,
    resistance: sr.resistance,
    btcRegime: btc,
    momentum5m: round2(m5),
    momentum15m: round2(m15)
  });
