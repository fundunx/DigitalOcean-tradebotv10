...
function dynamicScalpStopGbp(symbol) {
  ...
  const min = state.settings.scalp.dynamicStopMinGbp;
  const max = state.settings.scalp.dynamicStopMaxGbp;
...
function evaluateScalp(symbol) {
  ...
  if (mom3 > 0.02 && mom5 > 0.04) { longScore += 20; notes.push("micro momentum rising"); }
  if (mom3 < -0.02 && mom5 < -0.04) { shortScore += 20; notes.push("micro momentum falling"); }

  if (mom20 > 0.08) { longScore += 10; notes.push("short trend supports long"); }
  if (mom20 < -0.08) { shortScore += 10; notes.push("short trend supports short"); }

  if (sr.support && price <= sr.support * 1.004) { longScore += 12; notes.push("near support"); }
  if (sr.resistance && price >= sr.resistance * 0.996) { shortScore += 12; notes.push("near resistance"); }

  if (bb.ready && bb.squeeze && Math.abs(mom5) > 0.03) {
    if (mom5 > 0) longScore += 10;
    if (mom5 < 0) shortScore += 10;
    notes.push("Bollinger squeeze pressure");
  }

  if (bb.ready && bb.position === "LOWER" && mom3 > 0) { longScore += 8; notes.push("lower band bounce"); }
  if (bb.ready && bb.position === "UPPER" && mom3 < 0) { shortScore += 8; notes.push("upper band rejection"); }

  if (btc === "BTC_RISK_OFF") { longScore -= 18; shortScore += 6; notes.push("BTC risk-off"); }
  if (btc === "BTC_BULLISH") { longScore += 8; notes.push("BTC supportive"); }

  if (vol < 0.01) { longScore -= 10; shortScore -= 10; notes.push("movement too low"); }

  const bestSide = longScore >= shortScore ? "LONG" : "SHORT";
  const bestScore = Math.max(longScore, shortScore);
  let signal = bestSide;
  let confidence = Math.max(0, Math.min(95, Math.round(50 + bestScore + learning.confidenceBoost)));
  ...
  const data = review(symbol, "scalp", signal, reason, confidence, {
    price,
    strategy: "V7 WhatIf Scalp Brain",
...
function estimateExpectedNetProfitGbp(symbol, sizeGbp, pot) {
  const vol = volatilityPercent(symbol, 30);
  const expectedMovePercent = pot === "scalp"
    ? Math.max(0.25, vol * 0.9)
...
function canOpenTrade(pot, reviewData) {
  ...
  const minConfidence = pot === "scalp" ? 64 : 70;
  if (reviewData.confidence < minConfidence) {
    return { ok: false, reason: `Confidence ${reviewData.confidence}% below ${minConfidence}%` };
  }

  return { ok: true, reason: `${pot} risk checks passed` };
}
